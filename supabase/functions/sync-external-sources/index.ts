      ...
      try {
        ...
        if (existing) {
          if (existing.content_hash === contentHash) {
            ...
            await updateExistingDocument(existing.id, doc, contentHash, existing.version_number + 1);
            updatedCount++;
          }
        } else {
          // New document, insert
  ...
  try {
    ...
    for (const link of links) {
      ...
      if (title && href && title.length > 10) {
        documents.push({
          title: `NIST: ${title}`,
          content: await fetchPageContent(href.startsWith('http') ? href : `https://www.nist.gov${href}`),
...
async function scrapeMITREContent(source: any) {
  ...
  try {
    ...
    for (const technique of techniques.slice(0, 20)) { // Limit to 20 techniques
      documents.push({
...
async function fetchPageContent(url: string): Promise<string> {
  try {
    ...
    const elementsToRemove = doc.querySelectorAll('script, style, nav, footer, header, .navigation');
...
async function updateExistingDocument(docId: string, doc: any, contentHash: string, newVersion: number) {
