...
import { authenticateRequest, checkRateLimit } from '../_shared/auth-middleware.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    const authResult = await authenticateRequest(req, supabaseClient, 'consultant');
    ...
    console.log(`[CONSULTANT-RESPOND] Consultant ${userId} responding to escalation ${escalationId}`);
    ...
    console.log(`[CONSULTANT-RESPOND] Response sent successfully for escalation ${escalationId}`);
