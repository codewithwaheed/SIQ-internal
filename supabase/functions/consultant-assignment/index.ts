...
async function getAvailableConsultants(url: URL, authResult: any) {
  ...
  console.log('Finding available consultants:', { expertiseFilter, urgency, timezone });
  ...
  if (!consultants || consultants.length === 0) {
    console.warn('No consultants available for assignment:', { expertiseFilter, urgency });
...
async function handleAssignment(req: Request, authResult: any) {
  ...
  if (error) {
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to assign consultant', details: error.message }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
