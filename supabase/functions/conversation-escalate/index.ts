...
serve(async (req) => {
  return withSecurity(req, {
    ...
    try {
      ...
      const { conversationId, reason, priority = 'normal', aiInitiated = false }: ConversationEscalateRequest = await request.json();
    ...
    const { data: availableConsultants, error: consultantError } = await supabase
      .from('consultant_profiles')
      .select('user_id, id, expertise_areas, rating, total_escalations_handled')
      .eq('is_active', true)
    ...
    let waitMessage = "Your request has been escalated and will be reviewed by our team.";
    ...
    if (availableConsultants && availableConsultants.length > 0) {
      ...
      waitMessage = "You've been connected to a cybersecurity expert who will respond shortly.";
    } else {
      waitMessage = "All experts are currently busy. Your request has been queued and you'll receive a response within 24 hours.";
    }
    ...
    const { data: escalationData, error: escalationError } = await supabase
      ...
      .insert({
        ...
        reason: sanitizedReason || (aiInitiated ? 'AI-initiated escalation' : 'User requested human assistance'),
        priority: priority,
    ...
    if (assignedConsultantId) {
      ...
      const responseData = sanitizeResponse({
        ...
        message: waitMessage,
        estimatedWaitTime: availableConsultants?.length > 0 ? "< 30 minutes" : "< 24 hours",
        conversation_id: conversationId
      }, context.userRole);
