...
function calculateConsultantScore(consultant: any, inputs: RoutingInputs): ConsultantScore {
  ...
  const frameworkMatches = inputs.framework_tags.filter(tag => 
    consultant.primary_frameworks?.includes(tag) || 
    consultant.expertise_areas?.includes(tag)
  ).length;
  const frameworkScore = Math.min(frameworkMatches / Math.max(inputs.framework_tags.length, 1), 1) * ROUTING_WEIGHTS.framework_match;
  breakdown.framework_match = frameworkScore;
  totalScore += frameworkScore;

  // Industry experience match
  const industryScore = inputs.industry_context && consultant.industry_experience?.includes(inputs.industry_context) 
    ? ROUTING_WEIGHTS.industry_match : 0;
  ...
  const timezoneScore = inputs.timezone === consultant.timezone 
    ? ROUTING_WEIGHTS.timezone_match 
    : inputs.timezone && consultant.timezone ? ROUTING_WEIGHTS.timezone_match * 0.5 : 0;
  breakdown.timezone_match = timezoneScore;
  totalScore += timezoneScore;

  // Security clearance match
  const clearanceScore = inputs.clearance_requirement && 
    consultant.clearance_level && 
    consultant.clearance_level >= inputs.clearance_requirement 
    ? ROUTING_WEIGHTS.clearance_match : 0;
  ...
  const languageScore = consultant.languages?.includes(inputs.language_preference || 'en') 
    ? ROUTING_WEIGHTS.language_match : 0;
  ...
  // Satisfaction rating
  const satisfactionScore = (consultant.satisfaction_rating || 4.0) / 5.0 * ROUTING_WEIGHTS.satisfaction_rating;
...
async function getHistoricalScore(supabase: any, consultantId: string, userId: string): Promise<number> {
  try {
...
function generateAISummary(messages: any[], escalationData: any): string {
  ...
  const primaryConcern = userMessages[userMessages.length - 1]?.content?.substring(0, 200) || 'General cybersecurity inquiry';
...
serve(async (req) => {
  ...
  try {
    ...
    const contextPack = await generateContextPack(supabase, messages || [], userProfile, routingInputs);
