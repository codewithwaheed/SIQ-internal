...
serve(async (req) => {
  ...
  try {
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to fetch escalations" }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const consultantIds =
      escalations
        ?.filter((e) => e.assigned_consultant)
        .map((e) => e.assigned_consultant) || [];
    let consultantMap = {};
    ...
    const stats = {
      ...
      high_priority: escalationsData.filter((e) => e.priority === "high")
        .length,
    };
