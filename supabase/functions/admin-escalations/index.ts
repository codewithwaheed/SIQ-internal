...
serve(async (req) => {
  ...
  try {
    ...
    const consultantIds = escalations?.filter(e => e.assigned_consultant).map(e => e.assigned_consultant) || [];
    let consultantMap = {};
    ...
    if (consultantIds.length > 0) {
      ...
      consultantMap = consultants?.reduce((acc, consultant) => {
    ...
    const escalationsData: EscalationData[] = escalations?.map(escalation => {
      ...
      return {
        ...
        consultant_name: consultant ? `${consultant.first_name || ''} ${consultant.last_name || ''}`.trim() : undefined,
