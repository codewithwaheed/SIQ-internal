...
serve(async (req) => {
  ...
  try {
    ...
    if (!isSecurityRelated(question)) {
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "This assistant is specialized for cybersecurity questions only",
          suggestion:
            "Please ask questions related to cybersecurity, compliance, risk management, or information security.",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.log(
      `[CYBERSEC-AI] Processing query for user ${userId}: ${question.substring(0, 100)}...`,
    );
    ...
    const knowledgeContext = await getEnhancedKnowledgeContext(
      question,
      userId,
      cveContext,
    );
    ...
    const modelToUse = useFineTuned
      ? Deno.env.get("OPENAI_FINETUNED_MODEL") || model
      : model;

    console.log(
      `[CYBERSEC-AI] Using model: ${modelToUse} with ${knowledgeContext.totalSources} knowledge sources`,
    );
    ...
    try {
      ...
      if (!moderationResult.safe) {
        console.warn(
          "[CYBERSEC-AI] Content flagged by moderation:",
          moderationResult.reason,
        );
        answer =
          "I apologize, but I cannot provide this information as it may not align with responsible cybersecurity guidance. Please consult with a qualified cybersecurity professional for specific security concerns.";
      }
    } catch (error) {
      console.error(
        "[CYBERSEC-AI] OpenAI API failed, attempting fallback:",
        error,
      );
    ...
    console.log(
      `[CYBERSEC-AI] Query completed. Tokens used: ${aiResponse.usage?.total_tokens || "unknown"}`,
    );
    ...
    return new Response(
      JSON.stringify({
        ...
        error: error.message,
        fallbackAdvice:
          "I encountered an issue processing your cybersecurity question. Please consult with a cybersecurity professional for critical security matters.",
      }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
...
async function extractAndFetchCVEData(question: string) {
  ...
  for (const cveId of cveMatches.slice(0, 3)) {
    // Limit to 3 CVEs per query
    try {
      const response = await fetch(
        `${Deno.env.get("SUPABASE_URL")}/functions/v1/get-cve-details/${cveId}`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        if (result.success) {
          cveData.push({
            ...
            severity:
              result.cve.cvssV3?.baseSeverity ||
              result.cve.cvssV2?.baseSeverity ||
              "Unknown",
            score:
              result.cve.cvssV3?.baseScore ||
              result.cve.cvssV2?.baseScore ||
              "N/A",
...
async function getEnhancedKnowledgeContext(
  ...
  try {
    ...
    if (error || !data.success) {
      console.log(
        "[CYBERSEC-AI] Knowledge base query failed, proceeding without context",
      );
      return {
        summary:
          cveContext?.summary ||
    ...
    return {
      summary:
        contextSummary || "No specific knowledge base context available.",
...
function analyzeResponse(answer: string, question: string) {
  ...
  analysis.containsFrameworkRef = frameworks.some((fw) =>
    answer.toLowerCase().includes(fw.toLowerCase()),
  );
  ...
  analysis.containsActionableAdvice = actionWords.some((word) =>
    answer.toLowerCase().includes(word),
  );

  // Determine category
  if (
    answer.toLowerCase().includes("incident") ||
    answer.toLowerCase().includes("breach")
  ) {
    analysis.category = "incident_response";
    analysis.riskLevel = "high";
  } else if (
    answer.toLowerCase().includes("compliance") ||
    answer.toLowerCase().includes("audit")
  ) {
    analysis.category = "compliance";
  } else if (
    answer.toLowerCase().includes("vulnerability") ||
    answer.toLowerCase().includes("threat")
  ) {
  ...
  analysis.requiresExpertReview = criticalTopics.some(
    (topic) =>
...
function categorizeQuestion(question: string): string {
  ...
  if (
    ...
    lowerQ.includes("breach") ||
    lowerQ.includes("attack")
  ) {
    return "incident_response";
  } else if (
    lowerQ.includes("compliance") ||
    lowerQ.includes("audit") ||
    lowerQ.includes("standard")
  ) {
    return "compliance";
  } else if (
    lowerQ.includes("vulnerability") ||
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          console.log(
            `[CYBERSEC-AI] Rate limited, waiting ${waitTime}ms before retry ${attempt + 1}/${maxRetries}`,
          );
      ...
      if (attempt < maxRetries - 1) {
        ...
        console.log(
          `[CYBERSEC-AI] Attempt ${attempt + 1} failed, retrying in ${waitTime}ms:`,
          error.message,
        );
        await new Promise((resolve) => setTimeout(resolve, waitTime));
      }
...
function generateFallbackResponse(question: string): string {
  ...
  const fallbackResponses = {
    incident_response:
      "For incident response questions, I recommend following established incident response frameworks like NIST SP 800-61. Key steps include preparation, detection, containment, eradication, recovery, and lessons learned. For critical incidents, immediately engage your incident response team and consider external cybersecurity experts.",

    compliance:
      "For compliance-related questions, I suggest consulting the specific requirements of your applicable frameworks (NIST, ISO 27001, SOC 2, etc.). Ensure you have proper documentation, regular assessments, and continuous monitoring in place. Consider engaging a compliance consultant for detailed guidance.",
    ...
    access_control:
      "For access control questions, implement the principle of least privilege, regular access reviews, strong authentication (including MFA), and proper identity and access management (IAM) practices. Consider standards like NIST SP 800-63 for authentication guidance.",

    policy_governance:
      "For policy and governance matters, ensure you have comprehensive cybersecurity policies aligned with recognized frameworks, regular reviews and updates, proper approval processes, and staff training. Consider engaging legal and compliance experts for policy development.",

    general:
      "For general cybersecurity guidance, I recommend following established frameworks like the NIST Cybersecurity Framework, implementing defense-in-depth strategies, maintaining regular security assessments, and staying updated with current threat intelligence. For specific technical implementations, consult with qualified cybersecurity professionals.",
  };
...
async function getEnhancedKnowledgeContext(question: string, userId?: string) {
  try {
    ...
    const hasRelevantContent =
      filteredMasterResults.length > 0 || filteredUserResults.length > 0;

    if (!hasRelevantContent) {
      console.log(
        "[CYBERSEC-AI] No relevant content found above similarity threshold",
      );
      const fallbackContext = {
        summary:
          "No directly relevant knowledge base content found for this specific query.",
        sources: [],
        citations: [],
        totalSources: 0,
        masterKnowledge: [],
    ...
    const citations = [];
    const masterKnowledge = filteredMasterResults.map(
    ...
    const userDocuments = filteredUserResults.map(
      (result: any, index: number) => {
...
function buildEnhancedCybersecurityPrompt(knowledgeContext: any): string {
  const hasRelevantContext =
    knowledgeContext.totalSources > 0 && !knowledgeContext.belowThreshold;
...
function generateEnhancedFallbackResponse(
  ...
  if (knowledgeContext.totalSources > 0) {
    ...
    if (knowledgeContext.masterKnowledge.length > 0) {
      ...
      knowledgeContext.masterKnowledge
        .slice(0, 2)
        .forEach((source: any, i: number) => {
  ...
  return results
    .filter((result: any) => {
      ...
      if (hasDangerousContent) {
        console.warn(
          "[CYBERSEC-AI] Filtered out potentially malicious content",
        );
        return false;
      }
...
function buildContextSummary(
  ...
  if (masterResults.length > 0) {
    ...
    for (
      let i = 0;
      i < masterResults.length && tokenEstimate < maxTokens * 0.6;
      i++
    ) {
  ...
  if (userResults.length > 0 && tokenEstimate < maxTokens * 0.8) {
    ...
    for (
      let i = 0;
      i < userResults.length && tokenEstimate < maxTokens * 0.9;
      i++
    ) {
