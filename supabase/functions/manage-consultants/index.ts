...
async function getAvailableConsultants(
...
async function getConsultantStats(
  consultantId: string | null,
  authResult: any,
) {
  if (!consultantId) {
  ...
  if (
    ...
    consultant.user_id !== authResult.userId
  ) {
    return new Response(
  ...
  if (!isAdmin && (!isConsultant || !isOwnProfile)) {
    return new Response(
      JSON.stringify({
        ...
        details:
          "Only the consultant themselves or an admin can update this profile",
      }),
      {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
