...
async function listConsultants(searchParams: URLSearchParams, authResult: any) {
  ...
  if (authResult.userRole === 'business_owner') {
    ...
    query = query.eq('is_active', true).eq('is_verified', true).select(`
        id,
        user_id,
        bio,
        expertise_areas,
...
async function getAvailableConsultants(searchParams: URLSearchParams, authResult: any) {
  ...
  return new Response(JSON.stringify({ consultants: data }), {
    headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  });
}
...
async function getConsultantProfile(consultantId: string | null, authResult: any) {
  if (!consultantId) {
    return new Response(JSON.stringify({ error: 'Consultant ID is required' }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
  ...
  if (authResult.userRole === 'business_owner') {
    ...
    return new Response(JSON.stringify({ consultant: filteredData }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }

  return new Response(JSON.stringify({ consultant: data }), {
    headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  });
}
...
async function getConsultantStats(consultantId: string | null, authResult: any) {
  ...
  if (authResult.userRole !== 'admin' && consultant.user_id !== authResult.userId) {
    return new Response(JSON.stringify({ error: 'Access denied: Can only view own stats' }), {
      status: 403,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
  ...
  const { data: stats, error } = await supabase
    ...
    .eq(
      'escalation_id',
      'IN',
      supabase.from('escalations').select('id').eq('assigned_consultant', consultant.user_id),
    );
...
async function handlePostRequest(req: Request, authResult: any) {
  ...
  if (application_id) {
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({
          error: 'Failed to create consultant from application',
          details: error.message,
        }),
        {
          status: 500,
  ...
  if (!user_id) {
    return new Response(JSON.stringify({ error: 'user_id or application_id is required' }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
...
async function updateAvailability(consultantId: string, body: any, authResult: any) {
  ...
  if (!status || !['online', 'offline', 'busy', 'away'].includes(status)) {
    return new Response(JSON.stringify({ error: 'Valid availability status is required' }), {
      status: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
...
async function updateProfile(
  ...
  if (authResult.userRole !== 'admin') {
    const { is_verified, is_active, rating, total_escalations_handled, ...allowedData } =
      updateData;
...
async function handleDeleteRequest(url: URL, authResult: any) {
  ...
  return new Response(JSON.stringify({ message: 'Consultant deactivated successfully' }), {
    headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  });
}
