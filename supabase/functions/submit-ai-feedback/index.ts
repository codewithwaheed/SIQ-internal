...
serve(async (req) => {
  return withSecurity(req, {
    ...
    const validFeedbackTypes = [
      'ai_correct', 'ai_incorrect', 'ai_incomplete', 'ai_unhelpful', 
      'escalation_unnecessary', 'escalation_justified', 'other'
    ];
    ...
    const sanitizedSuggestedImprovement = suggested_improvement ? 
      InputSanitizer.sanitizeString(suggested_improvement, 2000) : null;
