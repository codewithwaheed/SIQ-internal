...
serve(async (req) => {
  return withSecurity(
    ...
    async (request: Request, context: SecurityContext) => {
      ...
      console.log(
        `[MANAGE-DOCUMENTS] ${request.method} request from user ${context.userId}`,
      );
              ...
              processing_status_counts: {
                completed:
                  documents?.filter((d) => d.processing_status === "completed")
                    .length || 0,
                pending:
                  documents?.filter((d) => d.processing_status === "pending")
                    .length || 0,
                failed:
      ...
      if (request.method === "DELETE") {
        ...
        console.log(
          `Document deleted: ${documentId} by user ${context.userId}`,
        );
      ...
      if (request.method === "POST") {
        ...
        console.log(
          `Document reprocessing started: ${documentId} by user ${context.userId}`,
        );
