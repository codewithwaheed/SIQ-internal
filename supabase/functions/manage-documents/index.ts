...
serve(async (req) => {
  return withSecurity(req, {
    ...
    if (request.method === 'GET') {
      ...
      const processingStatus = InputSanitizer.sanitizeString(url.searchParams.get('status') || '', 50);
      ...
      if (processingStatus && ['pending', 'processing', 'completed', 'failed'].includes(processingStatus)) {
