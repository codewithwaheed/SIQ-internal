...
serve(async (req) => {
  ...
  try {
    ...
    const { data: canEscalate, error: usageError } = await supabaseClient
    ...
    if (!canEscalate) {
      throw new Error("Escalation limit reached for your subscription tier. Please upgrade to Premium for consultant escalations.");
    }

    // Parse request body
    const { sessionId, messageLog, reason, priority = 'normal' }: EscalationRequest = await req.json();
    ...
    const { error: incrementError } = await supabaseClient
      .rpc('increment_escalation_count', { user_email: user.email });
    ...
    return new Response(JSON.stringify({
      ...
      message: "Your request has been escalated to our cybersecurity consultants. You'll hear back within 24 hours."
    }), {
