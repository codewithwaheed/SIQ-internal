...
function determineEscalationType(userQuery: string, messageLog: any[]): string {
  const allText = (
    userQuery +
    " " +
    messageLog.map((m) => m.content).join(" ")
  ).toLowerCase();
...
async function matchConsultant(
  ...
  const candidateConsultants =
    matchingConsultants.length > 0 ? matchingConsultants : consultants;
...
serve(async (req) => {
  ...
  try {
    ...
    const escalationType = determineEscalationType(
      userQuery || "",
      messageLog || [],
    );
    ...
    const { error: consultantUpdateError } = await supabaseClient
      ...
      .update({
        ...
        total_escalations_handled:
          selectedConsultant.total_escalations_handled + 1,
      })
