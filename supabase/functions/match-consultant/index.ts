...
const EXPERTISE_KEYWORDS = {
  ...
  'incident-response': ['incident', 'breach', 'attack', 'malware', 'virus', 'compromise', 'forensics'],
...
async function matchConsultant(supabaseClient: any, escalationType: string, userQuery: string) {
  ...
  const matchingConsultants = consultants.filter(consultant => 
    ...
    consultant.expertise_areas.includes(escalationType)
  );
...
serve(async (req) => {
  ...
  try {
    ...
    const selectedConsultant = await matchConsultant(supabaseClient, escalationType, userQuery || '');
    ...
    if (consultantUpdateError) {
      logStep("Warning: Failed to update consultant stats", { error: consultantUpdateError.message });
    }
    ...
    if (updatedEscalation.session_id) {
      ...
      if (messageError) {
        logStep("Warning: Failed to add system message", { error: messageError.message });
      }
    }
    ...
    try {
      ...
      logStep("Warning: Failed to send consultant notification", { error: notificationError });
    }
