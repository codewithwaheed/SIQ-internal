...
import { extractIPAddress, sanitizeError, validateRequestInput } from "../_shared/auth-middleware.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const validation = validateRequestInput(requestBody, {
      ...
      user_id: { required: false, type: 'string', pattern: /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i },
      tenant_id: { required: false, type: 'string', pattern: /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i }
    });
