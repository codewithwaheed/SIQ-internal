  ...
  try {
    ...
    if (url.pathname.endsWith('/usage')) {
      ...
      const usageData = {
        ...
        topResponses:
          responseUsage?.sort((a, b) => (b.use_count || 0) - (a.use_count || 0)).slice(0, 5) || [],
        topLinks:
          linkUsage?.sort((a, b) => (b.access_count || 0) - (a.access_count || 0)).slice(0, 5) ||
          [],
      };
    ...
    if (url.pathname.endsWith('/activity')) {
      ...
      const activity =
        recentLogs?.map((log) => {
          ...
          else if (log.action.includes('KNOWLEDGE') || log.action.includes('DOCUMENT'))
            type = 'content';
