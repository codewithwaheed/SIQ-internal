...
serve(async (req) => {
  ...
  try {
    ...
    if (url.pathname.endsWith('/trends')) {
      ...
      const monthlyData: Record<string, { chats: number; escalations: number }> = {};
      
      conversations?.forEach(conversation => {
        const month = new Date(conversation.created_at).toLocaleDateString('en-US', { 
          year: 'numeric', 
          month: 'short' 
        });
      ...
      escalations?.forEach(escalation => {
        const month = new Date(escalation.created_at).toLocaleDateString('en-US', { 
          year: 'numeric', 
          month: 'short' 
        });
    ...
    if (url.pathname.endsWith('/frameworks')) {
      ...
      escalations?.forEach(escalation => {
        if (escalation.framework_tags && Array.isArray(escalation.framework_tags)) {
    ...
    if (url.pathname.endsWith('/usage')) {
      ...
      const daysBack = timeframeParam === '7d' ? 7 : timeframeParam === '30d' ? 30 : 90;
      ...
      const usageData = {
        ...
        totalResponseUses: responseUsage?.reduce((sum, r) => sum + (r.use_count || 0), 0) || 0,
        totalLinkAccesses: linkUsage?.reduce((sum, l) => sum + (l.access_count || 0), 0) || 0,
        topResponses: responseUsage?.sort((a, b) => (b.use_count || 0) - (a.use_count || 0)).slice(0, 5) || [],
        topLinks: linkUsage?.sort((a, b) => (b.access_count || 0) - (a.access_count || 0)).slice(0, 5) || [],
      };
    ...
    if (url.pathname.endsWith('/response-times')) {
      ...
      logs?.forEach(log => {
        ...
        const responseTime = log.metadata?.response_time_ms ? log.metadata.response_time_ms / 1000 : 2.5;
        
        if (!dailyTimes[day]) dailyTimes[day] = [];
        dailyTimes[day].push(responseTime);
      });

      const responseTimeData = Object.entries(dailyTimes).map(([day, times]) => ({
        day,
        avgTime: times.length > 0 ? Number((times.reduce((a, b) => a + b, 0) / times.length).toFixed(1)) : 2.5
      }));
    ...
    if (url.pathname.endsWith('/activity')) {
      ...
      const activity = recentLogs?.map(log => {
        ...
        else if (log.action.includes('USER') || log.action.includes('AUTH')) type = 'user';
        else if (log.action.includes('KNOWLEDGE') || log.action.includes('DOCUMENT')) type = 'content';
...
function getFrameworkColor(framework: string): string {
  ...
  const key = Object.keys(colors).find(k => framework.toLowerCase().includes(k.toLowerCase()));
