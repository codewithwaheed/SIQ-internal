...
serve(async (req) => {
  ...
  try {
    ...
    // Get tier limits
    const { data: limits, error: limitsError } = await supabaseClient.rpc(
      "get_tier_limits",
      { tier_name: subscriptionData.subscription_tier },
    );
    ...
    const uploadsRemaining =
      tierLimits.upload_limit === -1
        ? -1 // unlimited
        : Math.max(
            0,
            tierLimits.upload_limit - subscriptionData.monthly_uploads_used,
          );

    const escalationsRemaining =
      tierLimits.escalation_limit === -1
        ? -1 // unlimited
        : Math.max(
            0,
            tierLimits.escalation_limit -
              subscriptionData.monthly_escalations_used,
          );

    // Check if user can perform actions
    const { data: canUpload } = await supabaseClient.rpc(
    ...
    const { data: canEscalate } = await supabaseClient.rpc(
      "can_user_perform_action",
      {
        user_email: user.email,
