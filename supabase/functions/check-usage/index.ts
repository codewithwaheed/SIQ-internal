...
serve(async (req) => {
  ...
  try {
    ...
    // Get tier limits
    const { data: limits, error: limitsError } = await supabaseClient.rpc('get_tier_limits', {
    ...
    const { data: canUpload } = await supabaseClient.rpc('can_user_perform_action', {
      user_email: user.email,
      action_type: 'upload',
    });

    const { data: canEscalate } = await supabaseClient.rpc('can_user_perform_action', {
      user_email: user.email,
