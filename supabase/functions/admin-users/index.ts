...
serve(async (req) => {
  return withSecurity(req, {
    ...
    console.log(`[ADMIN-USERS] ${request.method} request from admin ${context.userId}`);
    ...
    if (method === 'GET') {
      ...
      const search = InputSanitizer.sanitizeString(url.searchParams.get('search') || '', 100);
      ...
      try {
        ...
        if (error) {
          ...
          if (search) {
            countQuery = countQuery.or(`email.ilike.%${search}%,first_name.ilike.%${search}%,last_name.ilike.%${search}%,company_name.ilike.%${search}%`);
          }
    ...
    if (method === 'PATCH') {
      ...
      console.log(`User ${userId} updated by admin ${context.userId}:`, { role, status });
