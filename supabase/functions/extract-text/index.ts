...
serve(async (req) => {
  ...
  try {
      ...
      const extractionMethods = [
        ...
        () => {
          ...
          while ((match = streamPattern.exec(text)) !== null) {
            ...
            const readableText = streamContent.match(
              /[a-zA-Z0-9\s.,!?;:\-'"]{3,}/g,
            );
            if (readableText) {
      ...
      for (const method of extractionMethods) {
        try {
          ...
          if (result && result.length > 50) {
            ...
            console.log(
              "Extraction method successful, text length:",
    ...
    if (!extractedText || extractedText.length < 10) {
      throw new Error(
        "No meaningful text could be extracted from the document",
      );
    }
