...
const EVAL_PROMPTS = [
  ...
  {
    id: "high_risk_breach",
    prompt: "We just discovered unauthorized access to our payment processing systems. What should we do immediately?",
  ...
  {
    id: "compliance_gap",
    prompt: "Our organization needs to achieve SOC 2 Type II compliance but we don't have formal incident response procedures. How critical is this gap?",
  ...
  },
  {
    ...
    prompt: "We're a defense contractor needing CMMC Level 2 and also want SOC 2 compliance. There seems to be conflicting requirements around data encryption. Help!",
  ...
  {
    id: "iso27001_audit",
    prompt: "We're preparing for an ISO 27001 audit next month. What are the most commonly failed control areas?",
  ...
  },
  {
    ...
    prompt: "CISA just released an emergency directive about a critical vulnerability in our core infrastructure. We're not sure if we're affected or what to do.",
    expected_frameworks: ["NIST"],
    expected_risk: "high",
    should_escalate: true
  },
  {
    id: "policy_review",
    prompt: "Can you provide a checklist for reviewing our information security policies?",
  ...
  try {
    ...
    for (const evalPrompt of EVAL_PROMPTS) {
      try {
        ...
        const { data: chatResponse, error: chatError } = await supabaseClient.functions.invoke('chat-with-ai', {
          headers: {
        ...
        if (chatResponse.structured_response?.framework_tags) {
          const foundFrameworks = chatResponse.structured_response.framework_tags;
          ...
          evaluation.scores.framework_accuracy = correctFrameworks.length / expectedFrameworks.length;
        } else {
        ...
        if (chatResponse.structured_response?.risk_level) {
          evaluation.scores.risk_level_match = 
            chatResponse.structured_response.risk_level === evalPrompt.expected_risk ? 1 : 0;
        } else {
        ...
        evaluation.scores.escalation_accuracy = 
          (chatResponse.escalate_recommendation === evalPrompt.should_escalate) ? 1 : 0;

        // Citation check (basic)
        evaluation.scores.has_citations = 
          (chatResponse.message.includes('[') && chatResponse.message.includes(']')) ? 1 : 0;
        ...
        logStep(`Failed to evaluate prompt ${evalPrompt.id}`, { error: promptError.message });
    ...
    const aggregateScores = {
      json_validity_rate: results.reduce((sum, r) => sum + r.scores.json_valid, 0) / results.length,
      framework_accuracy_avg: results.reduce((sum, r) => sum + r.scores.framework_accuracy, 0) / results.length,
      risk_level_accuracy: results.reduce((sum, r) => sum + r.scores.risk_level_match, 0) / results.length,
      escalation_accuracy: results.reduce((sum, r) => sum + r.scores.escalation_accuracy, 0) / results.length,
      citation_rate: results.reduce((sum, r) => sum + r.scores.has_citations, 0) / results.length,
      appropriate_length_rate: results.reduce((sum, r) => sum + r.scores.length_appropriate, 0) / results.length,
      next_actions_rate: results.reduce((sum, r) => sum + r.scores.has_next_actions, 0) / results.length
    };
