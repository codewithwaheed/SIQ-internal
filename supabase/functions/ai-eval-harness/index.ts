  ...
  try {
    ...
    for (const evalPrompt of EVAL_PROMPTS) {
      try {
        ...
        const { data: chatResponse, error: chatError } = await supabaseClient.functions.invoke(
          'chat-with-ai',
          {
            headers: {
    ...
    const aggregateScores = {
      ...
      framework_accuracy_avg:
        results.reduce((sum, r) => sum + r.scores.framework_accuracy, 0) / results.length,
      risk_level_accuracy:
        results.reduce((sum, r) => sum + r.scores.risk_level_match, 0) / results.length,
      escalation_accuracy:
        results.reduce((sum, r) => sum + r.scores.escalation_accuracy, 0) / results.length,
      citation_rate: results.reduce((sum, r) => sum + r.scores.has_citations, 0) / results.length,
      appropriate_length_rate:
        results.reduce((sum, r) => sum + r.scores.length_appropriate, 0) / results.length,
      next_actions_rate:
        results.reduce((sum, r) => sum + r.scores.has_next_actions, 0) / results.length,
    };
