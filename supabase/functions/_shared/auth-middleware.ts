...
export async function checkRateLimit(
  ...
  try {
    ...
    if (attemptCount >= maxAttempts) {
      console.warn(`Rate limit exceeded for ${identifier} on ${endpoint}: ${attemptCount}/${maxAttempts}`);
 ...
 */
export function validateRequestInput(input: any, schema: Record<string, any>): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
