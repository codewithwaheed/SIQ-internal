...
serve(async (req) => {
  ...
  try {
    ...
    const textContent =
      file.type === 'text/plain' ? await file.text() : `Binary file: ${file.name}`;
    ...
    if (dbError) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to store document metadata' }), {
        status: 500,
        headers: corsHeaders,
      });
    }
...
async function encryptSensitiveContent(
  ...
  const keyHash = Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256', exportedKey)))
    .map((b) => b.toString(16).padStart(2, '0'))
    .join('');
