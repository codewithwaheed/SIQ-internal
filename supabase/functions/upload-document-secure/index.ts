...
import { authenticateRequest, auditSecurityEvent, extractIPAddress, sanitizeError } from "../_shared/auth-middleware.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: canUpload, error: usageError } = await supabaseClient
    ...
    if (usageError) {
      throw new Error(`Failed to check upload permissions: ${usageError.message}`);
    }
    ...
    if (!canUpload) {
      ...
      return new Response(
        JSON.stringify({ error: 'Upload limit exceeded for your subscription tier' }),
    ...
    const { error: validationError } = await supabaseClient.rpc('validate_file_upload', {
      file_name: file.name,
      file_size: file.size,
      file_type: file.type,
    ...
    if (validationError) {
      ...
      return new Response(
        JSON.stringify({ error: `File validation failed: ${validationError.message}` }),
        { status: 400, headers: corsHeaders }
      );
    }

    // Enhanced encryption for sensitive document content
    const textContent = file.type === 'text/plain' ? await file.text() : `Binary file: ${file.name}`;
      ...
      if (authHeader) {
        ...
        if (user) {
          await auditSecurityEvent(
            ...
            { error: sanitizeError(error), timestamp: new Date().toISOString() },
    ...
    return new Response(
      JSON.stringify({ error: sanitizeError(error, 'Failed to upload document') }),
      { status: 500, headers: corsHeaders }
    );
  }
});
...
// Enhanced encryption for sensitive document content
async function encryptSensitiveContent(content: string): Promise<{keyHash: string, iv: string, encrypted: string}> {
  ...
  const keyHash = Array.from(new Uint8Array(
    await crypto.subtle.digest('SHA-256', exportedKey)
