...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[RESOLVE-ESCALATION] Resolving escalation ${escalationId} by ${resolvedBy || 'user'}`);
    ...
    if (escalation.session_id) {
      const systemMessage = resolvedBy === 'consultant' 
    ...
    console.log(`[RESOLVE-ESCALATION] Escalation ${escalationId} resolved successfully`);
