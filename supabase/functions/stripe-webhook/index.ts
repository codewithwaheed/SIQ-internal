...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      const webhookTimestamp = parseInt(signature.split('t=')[1]?.split(',')[0] || '0');
      const currentTimestamp = Math.floor(Date.now() / 1000);
      const maxAge = 300; // 5 minutes
      
      if (Math.abs(currentTimestamp - webhookTimestamp) > maxAge) {
        console.error('Webhook timestamp too old:', { webhookTimestamp, currentTimestamp });
        return new Response('Webhook too old', { status: 400, headers: corsHeaders });
      }
    } catch (err) {
    ...
    console.log(`Processing Stripe event: ${event.type} (received in ${Date.now() - startTime}ms)`);
    ...
    switch (event.type) {
      ...
      case 'invoice.payment_succeeded':
        await handleSuccessfulPayment(event.data.object as Stripe.Invoice, supabaseClient);
        break;
      
      case 'invoice.payment_failed':
        await handleFailedPayment(event.data.object as Stripe.Invoice, supabaseClient);
        break;
...
async function handleSubscriptionChange(subscription: Stripe.Subscription, supabaseClient: any) {
  ...
  try {
    ...
    if (error) {
      ...
      console.log(`Updated subscription for ${customer.email} to ${subscriptionTier}`);
...
async function handleSuccessfulPayment(invoice: Stripe.Invoice, supabaseClient: any) {
...
async function handleFailedPayment(invoice: Stripe.Invoice, supabaseClient: any) {
