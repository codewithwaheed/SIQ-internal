export const DEFAULTS = {
  ...
  intro: "Strong password practices are fundamental to information security and protecting organizational assets from unauthorized access.",
  ...
  consequences: "Violations may result in disciplinary action up to and including termination of employment or contract, and may result in civil or criminal liability.",
...
export const POLICY_DEFAULTS = {
  ...
  next_review_date: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }),
  
  // Standard policy sections
  intro: 'Strong password practices are fundamental to information security and protecting organizational assets from unauthorized access.',
  purpose: 'To protect organizational assets and ensure regulatory compliance.',
  scope: 'This policy applies to all employees, contractors, and third parties with access to organizational resources.',
  ...
  // Consequences
  consequences: 'Violations may result in disciplinary action up to and including termination, and may also result in civil or criminal liability.',
...
export type ResolvedDefaults = {
  [K in keyof typeof DEFAULTS]: typeof DEFAULTS[K] extends () => infer R ? R : typeof DEFAULTS[K];
};
