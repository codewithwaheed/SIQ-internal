...
export class AuditLogger {
   ...
   */
  static async logAuth(userId: string, action: 'LOGIN_SUCCESS' | 'LOGIN_FAILURE' | 'LOGOUT' | 'SIGNUP', details?: string) {
    return this.log({
      userId,
      action,
  ...
  static async logEscalation(
    userId: string, 
    action: 'ESCALATION_REQUESTED' | 'ESCALATION_ASSIGNED' | 'ESCALATION_RESOLVED', 
    escalationId: string,
    details?: string,
    consultantId?: string
  ) {
    return this.log({
      userId,
      action,
      description: details || `Escalation ${action.toLowerCase().replace('escalation_', '')} (ID: ${escalationId})`,
