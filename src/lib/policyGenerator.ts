import { findMissingPlaceholders, groupMissingFields, humanizeFieldName } from './placeholderScanner';
...
export function detectPolicyIntent(text: string): "none" | "unspecified" | "specified" {
  ...
  const hasCreationVerb = /\b(create|draft|generate|make|build|write|produce|develop)\b/i.test(normalizedText);
...
export function generatePolicy(
  ...
  const remainingPlaceholders = Array.from(
    new Set((filledPolicy.match(/\{\{([a-z0-9_]+)\}\}/gi) || []).map(match => 
      match.replace(/[{}]/g, '')
    ))
  );
 ...
 */
export function getPolicyTypeFromInput(userInput: string): { type: PolicyType; title: string } {
  const slug = findPolicySlug(userInput);
  
  if (slug && slug in POLICY_TEMPLATES) {
    return { type: slug as PolicyType, title: POLICY_TEMPLATES[slug as PolicyType].title };
  }
...
function extractPolicyTitle(userInput: string): string {
  ...
  let title = normalized
    .replace(/^(create|generate|make|build|draft|write|produce|need|want|help.*draft|help.*create)\s+/i, '')
