...
const System = () => {
  ...
  const fetchSystemData = async () => {
    ...
    try {
      ...
      const { data: healthData, error: healthError } = await supabase.functions.invoke('admin-system', {
        body: { action: 'health' }
      });
      ...
      const { data: securityData, error: securityError } = await supabase
        ...
        .in('setting_name', ['two_factor_required', 'rate_limiting_enabled', 'audit_logging_enabled', 'session_timeout_enabled']);

      if (!securityError && securityData) {
        const settings = {
          twoFactor: securityData.find(s => s.setting_name === 'two_factor_required')?.setting_value 
            ? (securityData.find(s => s.setting_name === 'two_factor_required')?.setting_value as any)?.enabled || false : false,
          rateLimiting: securityData.find(s => s.setting_name === 'rate_limiting_enabled')?.setting_value 
            ? (securityData.find(s => s.setting_name === 'rate_limiting_enabled')?.setting_value as any)?.enabled || true : true,
          auditLogging: securityData.find(s => s.setting_name === 'audit_logging_enabled')?.setting_value 
            ? (securityData.find(s => s.setting_name === 'audit_logging_enabled')?.setting_value as any)?.enabled || true : true,
          sessionTimeout: securityData.find(s => s.setting_name === 'session_timeout_enabled')?.setting_value 
            ? (securityData.find(s => s.setting_name === 'session_timeout_enabled')?.setting_value as any)?.enabled || true : true
        };
    ...
    try {
      ...
      await supabase.functions.invoke('audit-log', {
        body: {
          ...
          metadata: { setting: dbSettingName, enabled: value, changed_by: user?.id }
        }
      });
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      {healthData?.network === 'healthy' 
                        ...
                        : loading ? 'System monitoring in progress...' : 'Unable to fetch system status'
                      }
                    </AlertDescription>
                  </Alert>
                        ...
                        <Card>
                          ...
                          <CardContent>
                            <div className="text-2xl font-bold">
                              <Badge variant={healthData?.network === 'healthy' ? "default" : "secondary"}>
                          ...
                          <CardContent className="space-y-4">
                            <div className="flex items-center justify-between">
                              ...
                              <Switch 
                                ...
                                onCheckedChange={(checked) => handleSecurityToggle('twoFactor', checked)}
                              />
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <Switch 
                                ...
                                onCheckedChange={(checked) => handleSecurityToggle('rateLimiting', checked)}
                              />
                            </div>
                          ...
                          <CardContent className="space-y-4">
                            <div className="space-y-2">
                              <div className="flex justify-between items-center">
                                ...
                                <Badge variant="default">
                                  {healthData?.backup?.lastBackup ? new Date(healthData.backup.lastBackup).toLocaleString() : 'Never'}
                                </Badge>
                              </div>
                              <div className="flex justify-between items-center">
                                <span className="text-sm font-medium">Backup Size</span>
                                <span className="text-sm">{healthData?.backup?.size || 'N/A'}</span>
                              </div>
                              <div className="flex justify-between items-center">
                                <span className="text-sm font-medium">Retention</span>
                                <span className="text-sm">{healthData?.backup?.retention || '30 days'}</span>
                              </div>
                            </div>
                            ...
                            <div className="pt-4 border-t space-y-2">
                              ...
                              <div className="text-sm text-muted-foreground">
                                <p>Next scheduled: {healthData?.backup?.nextScheduled || 'Sunday 2:00 AM UTC'}</p>
