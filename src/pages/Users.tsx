...
import { Search, UserPlus, Settings, Shield, Users as UsersIcon, Mail, RefreshCw, AlertCircle } from "lucide-react";
...
const Users = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://xfdqnmtzuuphxivsgmua.supabase.co/functions/v1/admin-users?${params}`, {
        method: 'GET',
        headers: {
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    try {
      ...
      const response = await fetch(`https://xfdqnmtzuuphxivsgmua.supabase.co/functions/v1/admin-users/${userId}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
          'Content-Type': 'application/json',
  ...
  const EmptyState = () => (
    <div className="text-center py-8">
      ...
      <p className="text-muted-foreground mb-4">
        {searchQuery ? 'Try adjusting your search terms.' : 'Get started by inviting your first user.'}
      </p>
                        ...
                        <Card key={user.id}>
                          <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                <Avatar>
                                  <AvatarImage src={`https://api.dicebear.com/6/initials/svg?seed=${user.first_name} ${user.last_name}`} />
                                  <AvatarFallback>
                                    {((user.first_name || '') + ' ' + (user.last_name || '')).split(' ').map(n => n[0]).join('').toUpperCase() || user.email[0].toUpperCase()}
                                  </AvatarFallback>
                                </Avatar>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div className="text-right text-sm">
                                  ...
                                  <div>{user.last_active_at ? new Date(user.last_active_at).toLocaleDateString() : 'Never'}</div>
                                </div>
                              </div>
                            </div>
                          </CardContent>
