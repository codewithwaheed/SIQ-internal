...
const EscalationQueue = () => {
  ...
  const updateEscalationStatus = async (escalationId: string, status: string, assignedTo?: string) => {
    try {
      const updates: any = { 
        status,
        ...
        <div className="lg:col-span-2">
          <Tabs defaultValue="all" className="space-y-4">
            <TabsList>
              ...
              <TabsTrigger value="pending">Pending ({filterEscalations('pending').length})</TabsTrigger>
              <TabsTrigger value="in_progress">In Progress ({filterEscalations('in_progress').length})</TabsTrigger>
              <TabsTrigger value="resolved">Resolved ({filterEscalations('resolved').length})</TabsTrigger>
            </TabsList>
                    ...
                    <CardHeader className="pb-3">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center space-x-3">
                           <Avatar>
                             <AvatarFallback>
                               {escalation.profiles?.first_name?.[0] || escalation.profiles?.email?.[0]?.toUpperCase() || 'U'}
                             </AvatarFallback>
                           </Avatar>
                          <div>
                             ...
                             <CardDescription className="text-xs">
                               {escalation.profiles?.company_name || 'No company'} • {getTimeAgo(escalation.created_at)}
                             </CardDescription>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Badge className={`text-xs text-white ${getPriorityColor(escalation.priority)}`}>
                            {escalation.priority}
                          </Badge>
                          <Badge className={`text-xs text-white ${getStatusColor(escalation.status)}`}>
            ...
            <Card>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex space-x-2">
                    {selectedEscalation.status === 'pending' && (
                      <Button 
                        size="sm" 
                        onClick={() => updateEscalationStatus(selectedEscalation.id, 'in_progress', user?.id)}
                      >
                        Take Case
                      </Button>
                    )}
                    {(selectedEscalation.status === 'in_progress' || selectedEscalation.status === 'assigned') && (
                      <Button 
                        size="sm" 
                        variant="outline"
                        onClick={async () => {
              ...
              <CardContent className="space-y-4">
                <div>
                  <h4 className="font-semibold text-sm mb-2">Client Information</h4>
                   <div className="text-sm space-y-1">
                     <p><span className="font-medium">Name:</span> {selectedEscalation.profiles?.first_name || 'N/A'} {selectedEscalation.profiles?.last_name || ''}</p>
                     <p><span className="font-medium">Email:</span> {selectedEscalation.profiles?.email || 'N/A'}</p>
                     <p><span className="font-medium">Company:</span> {selectedEscalation.profiles?.company_name || 'N/A'}</p>
                   </div>
                </div>

                <div>
                  <h4 className="font-semibold text-sm mb-2">Escalation Reason</h4>
                  ...
                  <div className="bg-muted p-3 rounded text-xs max-h-40 overflow-y-auto space-y-2">
                    {selectedEscalation.message_log && selectedEscalation.message_log.length > 0 ? (
                      selectedEscalation.message_log.map((message: any, index: number) => (
                        <div key={index} className={`p-2 rounded ${message.role === 'user' ? 'bg-primary/10' : 'bg-background'}`}>
                ...
                {selectedEscalation.status === 'in_progress' && (
                  <div>
                    <h4 className="font-semibold text-sm mb-2">Consultant Response</h4>
                    <Textarea
                      value={response}
                      ...
                      <Button 
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (error || !data.success) {
                              throw new Error(data?.error || error?.message || 'Failed to send response');
                            }
