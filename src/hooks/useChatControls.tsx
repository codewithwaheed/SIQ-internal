...
interface UseChatControlsReturn {
  ...
  waitForAIResponse: (conversationId: string, userMessageId: string, timeoutMs?: number) => Promise<boolean>;
...
export const useChatControls = (): UseChatControlsReturn => {
  ...
  const pollProcessingStatus = useCallback(async (conversationId: string): Promise<MessageProcessingStatus[]> => {
    try {
  ...
  const waitForAIResponse = useCallback(async (
  ...
  const simulateConsultantMessage = useCallback(async (
    conversationId: string, 
    content: string
  ): Promise<boolean> => {
    setLoading(true);
  ...
  const clearProcessingQueue = useCallback(async (conversationId: string): Promise<boolean> => {
    try {
