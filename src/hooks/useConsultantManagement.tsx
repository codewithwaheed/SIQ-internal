...
interface UseConsultantManagementReturn {
  ...
  createConsultant: (data: Partial<ConsultantProfile> & { user_id?: string; application_id?: string }) => Promise<boolean>;
  updateConsultantProfile: (consultantId: string, data: Partial<ConsultantProfile>) => Promise<boolean>;
...
export const useConsultantManagement = (): UseConsultantManagementReturn => {
  ...
  const fetchConsultants = useCallback(async (filters = {}) => {
    setLoading(true);
    setError(null);
  ...
  const updateConsultantAvailability = useCallback(async (consultantId: string, status: string): Promise<boolean> => {
    try {
  ...
  const getAvailableConsultants = useCallback(async (expertiseFilter?: string[]): Promise<ConsultantProfile[]> => {
    try {
      const params = new URLSearchParams({
        action: 'available',
  ...
  const createConsultant = useCallback(async (data: Partial<ConsultantProfile> & { user_id?: string; application_id?: string }): Promise<boolean> => {
    try {
      const { error } = await supabase.functions.invoke('manage-consultants', {
        method: 'POST',
        headers: {
  ...
  const updateConsultantProfile = useCallback(async (consultantId: string, data: Partial<ConsultantProfile>): Promise<boolean> => {
    try {
      const { error } = await supabase.functions.invoke('manage-consultants', {
        method: 'PUT',
        headers: {
  ...
  const deactivateConsultant = useCallback(async (consultantId: string): Promise<boolean> => {
    try {
      const { error } = await supabase.functions.invoke('manage-consultants', {
        method: 'DELETE',
        headers: {
      ...
      setConsultants(prev => prev.map(consultant => 
        consultant.id === consultantId 
          ? { ...consultant, is_active: false, availability_status: 'offline' as any }
          : consultant
      ));
