...
export const ConsultantDashboardMain = () => {
  ...
  const loadConversations = async () => {
    try {
      const { data, error } = await supabase.functions.invoke('admin-escalations', {
        body: { 
          action: 'list',
      ...
      const conversationData = data.escalations.map((escalation: any) => ({
        ...
        title: escalation.chat_context?.slice(0, 50) + '...' || 'Untitled Conversation',
      ...
      // Calculate stats
      const active = conversationData.filter((c: Conversation) => c.status === 'active').length;
      const pending = conversationData.filter((c: Conversation) => c.status === 'pending').length;
      const resolved = conversationData.filter((c: Conversation) => c.status === 'resolved').length;
      ...
      {selectedConversation ? (
        ...
        <>
          ...
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">Manage your assigned conversations and provide expert support.</p>
          </div>
