...
export const BillingSummary = () => {
  ...
  const fetchBillingData = async () => {
    try {
      ...
      const currentMonthPayouts = escalationsWithProfiles.map((escalation) => {
        const payoutAmount =
          payoutRates[escalation.priority as keyof typeof payoutRates] || payoutRates.normal;
        return {
          id: escalation.id,
      ...
      const totalEarnings = currentMonthPayouts.reduce(
        (sum, payout) => sum + payout.payout_amount,
        0,
      );
  ...
  const getGrowthPercentage = () => {
    ...
    return (
      ((stats.currentMonthEscalations - stats.lastMonthEscalations) / stats.lastMonthEscalations) *
      100
    );
  };
  ...
  return (
    <DashboardLayout title="Billing Summary" subtitle="Track your escalation payouts and earnings">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            ...
            <CardContent>
              ...
              <div className="flex items-center text-xs text-muted-foreground">
                {getGrowthPercentage() >= 0 ? (
                  ...
                  <TrendingUp className="mr-1 h-3 w-3 rotate-180 text-destructive" />
                )}
          ...
          <Card className="border-l-4 border-l-success">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Based on completed escalations</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-l-4 border-l-warning">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">From unresolved escalations</p>
            </CardContent>
          </Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {Object.entries(payoutRates).map(([priority, amount]) => (
                <div key={priority} className="rounded-lg border p-4 text-center">
                  <Badge
                    className={
                      priority === 'urgent'
                        ? 'bg-destructive'
        ...
        <Card>
          ...
          <CardContent>
            {escalationPayouts.length === 0 ? (
              <p className="py-8 text-center text-muted-foreground">No escalations this month</p>
            ) : (
                  ...
                  >
                    <div className="flex-1">
                      <div className="mb-1 flex items-center space-x-2">
                        <Badge
                          className={
                            payout.priority === 'urgent'
                              ? 'bg-destructive'
                              : payout.priority === 'high'
                                ? 'bg-warning'
                                : payout.priority === 'medium'
                                  ? 'bg-accent'
                                  : 'bg-muted'
                          }
                        >
            ...
            <div className="space-y-4 py-8 text-center">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  Direct payout tracking and automated payments will be available once Stripe is
                  integrated.
                </p>
              </div>
