...
export const BillingSummary = () => {
  ...
  const [escalationPayouts, setEscalationPayouts] = useState<
    EscalationPayout[]
  >([]);
  ...
  const fetchBillingData = async () => {
    try {
      ...
      const currentMonthPayouts = escalationsWithProfiles.map((escalation) => {
        const payoutAmount =
          payoutRates[escalation.priority as keyof typeof payoutRates] ||
          payoutRates.normal;
        return {
          id: escalation.id,
          ...
          client_company:
            escalation.profiles?.company_name || "Unknown Company",
        };
      });
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `billing-summary-${new Date().toISOString().slice(0, 7)}.csv`,
    );
  ...
  const getGrowthPercentage = () => {
    if (stats.lastMonthEscalations === 0)
      return stats.currentMonthEscalations > 0 ? 100 : 0;
    return (
      ((stats.currentMonthEscalations - stats.lastMonthEscalations) /
        stats.lastMonthEscalations) *
      100
    );
  };
          ...
          <Card className="border-l-4 border-l-primary">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                This Month Escalations
              </CardTitle>
              <AlertTriangle className="h-4 w-4 text-primary" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {stats.currentMonthEscalations}
              </div>
                  ...
                  >
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        <Badge
                          variant={
                            payout.status === "resolved" ? "default" : "outline"
                          }
                          className={
                            payout.status === "resolved" ? "bg-success" : ""
                          }
                        >
                      ...
                      <p className="font-medium">
                        {payout.reason || "General Support Request"}
                      </p>
            ...
            <div className="text-center py-8 space-y-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  Direct payout tracking and automated payments will be
                  available once Stripe is integrated.
                </p>
              </div>
