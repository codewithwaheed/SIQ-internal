...
export const ConsultantDashboard = () => {
  ...
  const fetchEscalations = async () => {
    try {
      ...
      const resolved = escalationsWithProfiles.filter(
        (e) => e.status === "resolved",
      ).length;

      const pending = escalationsWithProfiles.filter(
        (e) => e.status === "pending",
      ).length;

      const followUp = escalationsWithProfiles.filter(
                    ...
                    <DialogTrigger asChild>
                      <div className="flex space-x-2">
                        <Button
                          ...
                          onClick={() =>
                            window.open(
                              `/dashboard/client/${escalation.user_id}`,
                              "_blank",
                            )
                          }
                        >
                        ...
                        <TabsContent value="chat" className="space-y-4">
                          <div className="bg-muted p-4 rounded-lg">
                            ...
                            <div className="max-h-64 overflow-y-auto space-y-2">
                              {escalation.message_log &&
                              escalation.message_log.length > 0 ? (
                                escalation.message_log.map(
                                  (message: any, index: number) => (
                                    <div
                                      key={index}
                                      className={`p-2 rounded ${message.role === "user" ? "bg-primary/10" : "bg-muted"}`}
                                    >
