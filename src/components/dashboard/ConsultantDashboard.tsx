...
export const ConsultantDashboard = () => {
  ...
  const fetchEscalations = async () => {
    try {
      ...
      const newThisWeek = escalationsWithProfiles.filter(
        (e) => new Date(e.created_at) >= weekAgo,
      ).length;

      const resolved = escalationsWithProfiles.filter((e) => e.status === 'resolved').length;

      const pending = escalationsWithProfiles.filter((e) => e.status === 'pending').length;

      const followUp = escalationsWithProfiles.filter((e) => e.status === 'in_progress').length;
                  ...
                  <div className="flex flex-1 items-center space-x-4">
                    ...
                    <div className="flex-1">
                      <div className="mb-1 flex items-center space-x-2">
                        ...
                        <Badge variant="outline">{escalation.status.replace('_', ' ')}</Badge>
                      </div>
                      <p className="text-sm font-medium">
                        {escalation.reason || 'General Support Request'}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {escalation.profile?.company_name &&
                          `${escalation.profile.company_name} • `}
                        {escalation.profile?.first_name} {escalation.profile?.last_name} •{' '}
                    ...
                    <DialogTrigger asChild>
                      <div className="flex space-x-2">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            window.open(`/dashboard/client/${escalation.user_id}`, '_blank')
                          }
                        >
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="space-y-1 text-sm">
                                <p>
                                  <strong>Name:</strong> {escalation.profile?.first_name}{' '}
                                  {escalation.profile?.last_name}
                                </p>
                                <p>
                                  <strong>Company:</strong>{' '}
                                  {escalation.profile?.company_name || 'Not provided'}
                                </p>
                                <p>
                                  <strong>Email:</strong> {escalation.profile?.email}
                                </p>
                              </div>
                            </div>
                            <div>
                              <h4 className="mb-2 font-medium">Escalation Info</h4>
                              <div className="space-y-1 text-sm">
                                <p>
                                  <strong>Priority:</strong> {escalation.priority}
                                </p>
                                <p>
                                  <strong>Status:</strong> {escalation.status}
                                </p>
                                <p>
                                ...
                                <p>
                                  <strong>Reason:</strong> {escalation.reason || 'Not specified'}
                                </p>
                              </div>
                            </div>
                          </div>
                            ...
                            <div className="max-h-64 space-y-2 overflow-y-auto">
                              {escalation.message_log && escalation.message_log.length > 0 ? (
                                escalation.message_log.map((message: any, index: number) => (
                                  <div
                                    key={index}
                                    className={`rounded p-2 ${message.role === 'user' ? 'bg-primary/10' : 'bg-muted'}`}
                                  >
                            ...
                            <div>
                              ...
                              <div className="flex space-x-2">
                                <Button
                                  variant="outline"
                                  onClick={() =>
                                    updateEscalationStatus(escalation.id, 'in_progress')
                                  }
                                >
                                  Mark In Progress
                                </Button>
