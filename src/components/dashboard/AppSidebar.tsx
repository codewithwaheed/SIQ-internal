  ...
  return (
    ...
    >
      ...
      <SidebarContent className="px-4 py-6">
        <SidebarGroup>
          {!collapsed && (
            <SidebarGroupLabel className="text-xs font-medium text-muted-foreground px-3 mb-3 uppercase tracking-wider">
              Navigation
            </SidebarGroupLabel>
          )}
          <SidebarGroupContent>
      ...
      <SidebarFooter className="border-t border-border/10 p-6">
        <div className="space-y-2">
          ...
          <button
            onClick={signOut}
            className={`flex items-center ${collapsed ? "justify-center" : "gap-3"} px-3 py-3 rounded-xl w-full text-left text-muted-foreground hover:bg-accent hover:text-foreground transition-all duration-200 group hover:shadow-sm`}
          >
            <LogOut
              className={`h-5 w-5 flex-shrink-0 transition-transform duration-200 ${collapsed ? "" : "group-hover:scale-110"}`}
            />
