...
export function FeatureGate({ 
  ...
  if (showUpgradePrompt) {
    return (
      <UpgradePrompt feature={feature} trigger={children} />
    );
  }
...
export function UpgradePrompt({ feature, trigger, open, onOpenChange }: UpgradePromptProps) {
  ...
  const getPricingContext = (tier: string) => {
    ...
    switch (tier) {
      case 'Pro':
        return { price: '$49', period: 'month', savings: 'Upload documents + 1 expert escalation' };
      case 'Premium':
        return { price: '$149', period: 'month', savings: 'Unlimited uploads + 2 live consultations' };
      default:
        return { price: '$0', period: 'month', savings: 'AI-powered cybersecurity guidance' };
    }
  };
...
export function InlineUpgradeNudge({ feature, className }: { feature: FeatureKey; className?: string }) {
  ...
  return (
    <div className={cn("flex items-center justify-between p-3 rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/30", className)}>
      <div className="flex items-center gap-2">
...
export function UsageIndicator({ feature }: { feature: FeatureKey }) {
  ...
  const usagePercentage = ((access.totalLimit - (access.remainingUsage || 0)) / access.totalLimit) * 100;
