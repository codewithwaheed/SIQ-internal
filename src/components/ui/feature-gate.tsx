        ...
        trigger={
          ...
          >
            ...
            {config.requiredTier === 'Pro' && (
              <Crown className="absolute -right-1 -top-1 h-3 w-3 text-blue-500" />
            )}
            {config.requiredTier === 'Premium' && (
              <Zap className="absolute -right-1 -top-1 h-3 w-3 text-purple-500" />
            )}
          </Button>
        }
      />
...
export function UpgradePrompt({ feature, trigger, open, onOpenChange }: UpgradePromptProps) {
  const { checkFeatureAccess, getFeatureConfig, getUpgradeUrl, currentTier } = useFeatureGating();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="animate-scale-in sm:max-w-lg">
        <DialogHeader className="text-center">
          ...
          <DialogTitle className="text-2xl font-bold">Unlock {config.name}</DialogTitle>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between rounded-lg border bg-muted/50 p-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">Current: {currentTier} Plan</span>
            </div>
          ...
          <div className="space-y-2 text-center">
            ...
            <p className="text-sm font-medium text-green-600">{pricing.savings}</p>
          </div>
          ...
          <DialogFooter className="flex-col gap-3 sm:flex-col">
            <Button
              ...
              className="h-11 w-full animate-fade-in bg-gradient-to-r from-blue-600 to-purple-600 text-base font-semibold hover:from-blue-700 hover:to-purple-700"
              size="lg"
            >
          ...
          <div className="text-center">
            <p className="text-xs text-muted-foreground">
              ✓ 30-day money-back guarantee • ✓ Cancel anytime • ✓ Secure billing
            </p>
          </div>
        </div>

        <UpgradeSelection open={showUpgradeSelection} onOpenChange={setShowUpgradeSelection} />
      </DialogContent>
    </Dialog>
  );
}
...
export function InlineUpgradeNudge({
  ...
  const { checkFeatureAccess, getFeatureConfig, getUpgradeUrl } = useFeatureGating();
  const navigate = useNavigate();
  const access = checkFeatureAccess(feature);
  const config = getFeatureConfig(feature);
  ...
  return (
    <div
      className={cn(
        'flex items-center justify-between rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-950/30',
        className,
      )}
    >
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-amber-600 dark:text-amber-400">{config.description}</p>
        </div>
      </div>
      <Button
        size="sm"
...
export function UsageIndicator({ feature }: { feature: FeatureKey }) {
  ...
  const usagePercentage =
    ((access.totalLimit - (access.remainingUsage || 0)) / access.totalLimit) * 100;
