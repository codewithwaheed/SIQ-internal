...
const AVAILABILITY_STATUS_CONFIG = {
  online: { label: 'Online', color: 'bg-green-500', textColor: 'text-green-600', icon: CheckCircle },
  offline: { label: 'Offline', color: 'bg-gray-500', textColor: 'text-gray-600', icon: XCircle },
  busy: { label: 'Busy', color: 'bg-red-500', textColor: 'text-red-600', icon: Activity },
  away: { label: 'Away', color: 'bg-yellow-500', textColor: 'text-yellow-600', icon: Clock }
};
...
export const ConsultantManagementDashboard = () => {
  ...
  const filteredConsultants = consultants.filter(consultant => {
    const matchesSearch = 
      ...
      consultant.expertise_areas.some(area => area.toLowerCase().includes(searchTerm.toLowerCase()));

    const matchesStatus = 
      statusFilter === 'all' || 
      consultant.availability_status === statusFilter;
                      ...
                      <TableCell>
                        <div className="flex space-x-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() => updateConsultantStatus(consultant.id, !consultant.is_active)}
                          >
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {selectedConsultant.hourly_rate ? `$${selectedConsultant.hourly_rate}` : 'Not set'}
                  </p>
                </div>
              </div>
              ...
              {selectedConsultant.bio && (
                <div>
                  <Label>Bio</Label>
                  <p className="text-sm text-muted-foreground mt-1">{selectedConsultant.bio}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="text-center">
                  <p className="text-2xl font-bold">{selectedConsultant.total_escalations_handled}</p>
                  <p className="text-xs text-muted-foreground">Total Cases</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold">{selectedConsultant.avg_response_time_hours.toFixed(1)}h</p>
                  <p className="text-xs text-muted-foreground">Avg Response</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold">{(selectedConsultant.success_rate * 100).toFixed(1)}%</p>
                  <p className="text-xs text-muted-foreground">Success Rate</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold">{selectedConsultant.client_feedback_score.toFixed(1)}</p>
