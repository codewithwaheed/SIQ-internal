...
export const ConsultantManagementDashboard = () => {
  ...
  useEffect(() => {
    if (userRole === "admin") {
      Promise.all([fetchConsultants(), fetchApplications()]).finally(() =>
        setLoading(false),
      );
    }
  }, [userRole]);
  ...
  const updateConsultantStatus = async (
    ...
    try {
      ...
      setConsultants((prev) =>
        prev.map((c) =>
          c.id === consultantId ? { ...c, is_active: isActive } : c,
        ),
      );
  ...
  const filteredConsultants = consultants.filter((consultant) => {
    const matchesSearch =
      consultant.profiles.first_name
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      consultant.profiles.last_name
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
    ...
    const matchesStatus =
      statusFilter === "all" || consultant.availability_status === statusFilter;
  ...
  const getStatusBadge = (status: string) => {
    const config =
      AVAILABILITY_STATUS_CONFIG[
        status as keyof typeof AVAILABILITY_STATUS_CONFIG
      ];
  ...
  const onlineConsultants = consultants.filter(
    (c) => c.availability_status === "online",
  ).length;
                    ...
                    <TableRow key={consultant.id}>
                      ...
                      <TableCell>
                        <div className="text-center">
                          <div className="font-medium">
                            {consultant.total_escalations_handled}
                          </div>
                          <div className="text-xs text-muted-foreground">
                      ...
                      <TableCell>
                        <div className="flex space-x-2">
                          ...
                          <Button
                            variant={
                              consultant.is_active ? "destructive" : "default"
                            }
                            size="sm"
                            onClick={() =>
                      ...
                      <TableCell>
                        {application.status === "approved" ? (
                          <Button
                            size="sm"
                            onClick={() =>
                              createConsultantFromApplication(application.id)
                            }
                          >
                            Create Profile
                          </Button>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {selectedConsultant.years_experience} years
                  </p>
                </div>
                <div>
