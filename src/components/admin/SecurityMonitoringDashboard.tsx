...
import { Shield, AlertTriangle, CheckCircle, XCircle, Eye, Clock, User } from 'lucide-react';
...
export const SecurityMonitoringDashboard: React.FC = () => {
  ...
  const loadSecurityData = async () => {
    try {
      ...
      const { data: config, error: configError } = await supabase.rpc('validate_security_config');
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-muted-foreground">Loading security monitoring data...</p>
        </div>
      </div>
    );
  }
                  ...
                  securityConfig.map((config, index) => (
                    <div key={index} className="rounded-lg border p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="mb-1 flex items-center gap-2">
                            <span className="font-medium">{config.check_name}</span>
                            <Badge variant={config.status === 'PASS' ? 'default' : 'destructive'}>
                              {config.status}
                            </Badge>
                          </div>
