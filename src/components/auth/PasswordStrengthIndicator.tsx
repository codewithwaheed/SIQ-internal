    ...
    <div className="mt-2 space-y-3">
      ...
      <div className="space-y-1">
        ...
        <div className="grid grid-cols-1 gap-1 text-xs">
          <RequirementItem met={password.length >= 14} text="At least 14 characters" />
          ...
          <div className="space-y-1">
            {validation.errors.map((error, index) => (
              <div key={index} className="flex items-start text-xs text-red-600">
                <X className="mr-1 mt-0.5 h-3 w-3 flex-shrink-0" />
                <span>{error}</span>
              </div>
            ))}
          </div>
      ...
      {password && PasswordValidator.checkCompromised(password) && (
        <div className="flex items-start rounded border border-amber-200 bg-amber-50 p-2 text-xs text-amber-600">
          <AlertTriangle className="mr-1 mt-0.5 h-3 w-3 flex-shrink-0" />
...
const RequirementItem = ({ met, text }: RequirementItemProps) => (
  <div className="flex items-center space-x-2">
    {met ? <Check className="h-3 w-3 text-green-600" /> : <X className="h-3 w-3 text-red-400" />}
