...
export const SensitiveDataDetector = ({ content, onContentMasked }: SensitiveDataDetectorProps) => {
  const [sensitiveData, setSensitiveData] = useState<Array<{type: string, value: string, start: number, end: number}>>([]);
  ...
  const patterns = [
    ...
    { type: 'API Key', regex: /(?:api[_-]?key|token|secret)['":\s]*[a-zA-Z0-9-_]{20,}/gi },
  ...
  const maskContent = () => {
    ...
    sortedData.forEach(item => {
      ...
      maskedContent = maskedContent.substring(0, item.start) + mask + maskedContent.substring(item.end);
    });
