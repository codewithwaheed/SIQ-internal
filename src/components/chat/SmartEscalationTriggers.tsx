...
const detectRepeatedQuestions = (messages: Message[]): boolean => {
  ...
  const hasQuestions = recentUserMessages.filter(
    (msg) => questionWords.some((word) => msg.includes(word + ' ')) || msg.includes('?'),
  );
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-2 flex items-center gap-2">
                    <h4 className="text-sm font-medium text-foreground">{trigger.title}</h4>
                    <Badge
                      variant="secondary"
                      className={cn('px-2 py-0.5 text-xs', getPriorityBadgeColor(trigger.priority))}
                    >
                      {trigger.priority}
                    </Badge>
                  </div>
