...
const detectRepeatedQuestions = (messages: Message[]): boolean => {
  ...
  const hasQuestions = recentUserMessages.filter(
    (msg) =>
      questionWords.some((word) => msg.includes(word + " ")) ||
      msg.includes("?"),
  );
...
const detectBlockedTasks = (content: string): boolean => {
  ...
  return blockageSignals.some((signal) =>
...
const detectSensitiveDocumentContext = (documents: any[]): boolean => {
  ...
  return documents.some((doc) =>
    sensitiveIndicators.some(
      (indicator) =>
        doc.file_name.toLowerCase().includes(indicator) ||
        (doc.content_extracted &&
          doc.content_extracted.toLowerCase().includes(indicator)),
    ),
  );
};
...
export const SmartEscalationTriggers = ({
  ...
  const analyzeTriggers = () => {
    ...
    const assistantMessagesSinceLastTrigger =
      conversationContext.messageCount - lastTriggerMessageCount;
    ...
    if (detectHighRiskScenarios(lastUserMessage.content)) {
      newTriggers.push({
        ...
        reason:
          "This appears to involve a potential security incident that requires immediate expert attention.",
    ...
    if (detectRegulatoryKeywords(lastUserMessage.content)) {
      newTriggers.push({
        ...
        reason:
          "Regulatory compliance questions often require specialized expertise and current knowledge of requirements.",
    ...
    if (detectSensitiveDocumentContext(uploadedDocuments)) {
      newTriggers.push({
        ...
        reason:
          "You've uploaded sensitive documents that may benefit from expert review and specific guidance.",
    ...
    if (detectUrgencyWords(lastUserMessage.content)) {
      newTriggers.push({
        ...
        reason:
          "Your message indicates this is time-sensitive and may require immediate expert assistance.",
    ...
    if (detectRepeatedQuestions(messages)) {
      newTriggers.push({
        ...
        reason:
          "Multiple follow-up questions suggest this topic might benefit from direct expert consultation.",
    ...
    if (detectBlockedTasks(lastUserMessage.content)) {
      newTriggers.push({
        ...
        reason:
          "It sounds like you're encountering implementation difficulties that an expert could help resolve.",
    ...
    const sessionDuration =
      (new Date().getTime() - sessionStart.getTime()) / (1000 * 60); // minutes
    if (sessionDuration > 15 && conversationContext.messageCount > 8) {
      newTriggers.push({
        ...
        reason:
          "You've been exploring this topic extensively. An expert consultation might provide more efficient guidance.",
    ...
    const filteredTriggers = newTriggers.filter(
      (trigger) =>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Badge
                      ...
                      className={cn(
                        "text-xs px-2 py-0.5",
                        getPriorityBadgeColor(trigger.priority),
                      )}
                    >
                      {trigger.priority}
                    </Badge>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <EscalationButton messages={messages} variant="inline" />
