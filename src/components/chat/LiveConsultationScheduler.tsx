      ...
      <Card className={cn("border-green-200 bg-green-50/50", className)}>
        ...
        <CardContent className="space-y-4">
          <div className="bg-white rounded-lg p-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary">
                {scheduledMeeting.meeting_type === 'video' && <Video className="h-3 w-3 mr-1" />}
                {scheduledMeeting.meeting_type === 'audio' && <Phone className="h-3 w-3 mr-1" />}
                {scheduledMeeting.meeting_type === 'screen_share' && <MessageSquare className="h-3 w-3 mr-1" />}
          ...
          {scheduledMeeting.meeting_url && (
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Input
                  value={scheduledMeeting.meeting_url}
                  readOnly
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <h4 className="text-sm font-medium text-blue-800 mb-1">Before the meeting:</h4>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="grid grid-cols-1 gap-3">
                {[
                  { value: 'video', label: 'Video Call', icon: Video, desc: 'Face-to-face discussion' },
                  { value: 'audio', label: 'Audio Only', icon: Phone, desc: 'Voice call (low bandwidth)' },
                  { value: 'screen_share', label: 'Screen Share', icon: MessageSquare, desc: 'Review documents together' }
                ].map((option) => (
                  <Button
                    key={option.value}
                    variant={meetingData.type === option.value ? "default" : "outline"}
                    className="h-auto p-4 justify-start"
                    onClick={() => setMeetingData(prev => ({ ...prev, type: option.value as any }))}
                  >
        ...
        {step === 'details' && (
          <div className="space-y-4">
            <div className="bg-muted/30 rounded-lg p-3">
              <div className="text-sm font-medium mb-1">Meeting Summary</div>
              <div className="text-sm text-muted-foreground">
                {format(selectedDate!, 'MMM d, yyyy')} at {selectedTime} ({meetingData.duration} min)
              </div>
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) => setMeetingData(prev => ({ ...prev, preparation_notes: e.target.value }))}
                rows={2}
              />
            </div>
