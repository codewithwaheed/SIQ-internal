...
test.describe("Security Audit", () => {
  test.describe("Authentication & Authorization", () => {
    test("should deny access to admin APIs with user token", async ({
      request,
      page,
    }) => {
    ...
    test("should validate CVE ID format properly", async ({
      request,
      page,
    }) => {
      await page.goto("/auth");
  ...
  test.describe("Cross-Site Scripting (XSS)", () => {
    test("should escape user input in chat messages", async ({ page }) => {
      ...
      for (const payload of xssPayloads) {
        ...
        const messageContent = await page.innerHTML(
