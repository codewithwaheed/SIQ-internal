...
test.describe('Security Report Generation', () => {
  test('generate comprehensive security report', async ({ page, request }) => {
    ...
    try {
      ...
      const hasError = await page.waitForSelector('[data-testid="error-message"]', { timeout: 5000 });
    ...
    try {
      ...
      report.categories.authorization.push({
        ...
        severity: response.status() === 401 || response.status() === 403 ? 'LOW' : 'CRITICAL'
      });
    } catch (error) {
      report.categories.authorization.push({
        category: 'Authorization',
    ...
    try {
      const response = await request.get(`/api/cve/${encodeURIComponent(xssPayload)}`);
    ...
    try {
      ...
      const isValidResponse = response.status() === 200 || response.status() === 401;
    ...
    try {
      ...
      await page.fill('[data-testid="chat-input"]', 'My credit card is 1234-5678-9012-3456');
      ...
      report.categories.dataProtection.push({
        ...
        status: messageContent?.includes('1234-5678-9012-3456') ? 'WARNING' : 'PASS',
        details: 'Check for proper handling of sensitive data in messages',
        severity: messageContent?.includes('1234-5678-9012-3456') ? 'MEDIUM' : 'LOW'
      });
    } catch (error) {
      report.categories.dataProtection.push({
        category: 'Data Protection',
    ...
    report.summary.warnings = allTests.filter(t => t.status === 'WARNING').length;

    // Determine overall status
    const criticalIssues = allTests.filter(t => t.severity === 'CRITICAL' && t.status === 'FAIL').length;
    const highIssues = allTests.filter(t => t.severity === 'HIGH' && t.status === 'FAIL').length;
    ...
    if (report.summary.failed > 0) {
      report.recommendations.push('Address all failed security tests before production deployment');
    }
    if (allTests.some(t => t.testName.includes('Security Headers') && t.status !== 'PASS')) {
      report.recommendations.push('Implement comprehensive security headers (CSP, HSTS, etc.)');
    }
    if (allTests.some(t => t.testName.includes('Input Validation') && t.status !== 'PASS')) {
      report.recommendations.push('Strengthen input validation and sanitization');
    }
    if (allTests.some(t => t.testName.includes('CVE') && t.status !== 'PASS')) {
      report.recommendations.push('Verify CVE API integration and error handling');
    }
...
<body>
    ...
    ${Object.entries(report.categories).map(([categoryName, tests]) => `
        <div class="category">
