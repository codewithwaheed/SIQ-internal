...
test.describe("Penetration Testing Suite", () => {
  test.describe("API Security Checklist", () => {
    ...
    test("should validate all input parameters", async ({ request }) => {
      ...
      for (const input of maliciousInputs) {
        // Test CVE endpoint
        const cveResponse = await request.get(
          `/api/cve/${encodeURIComponent(input)}`,
        );
  ...
  test.describe("OWASP ZAP Simulation", () => {
    test("should resist common attack vectors", async ({ page, request }) => {
      ...
      for (const vector of attackVectors) {
        ...
        const hasErrorMessage = await page.isVisible(
  ...
  test.describe("Rate Limiting and DoS Protection", () => {
    test("should implement rate limiting on authentication", async ({
      page,
    }) => {
      await page.goto("/auth");
      ...
      const earlyAttempts =
        failedAttempts.slice(0, 5).reduce((a, b) => a + b, 0) / 5;
      const laterAttempts =
        failedAttempts.slice(-5).reduce((a, b) => a + b, 0) / 5;
