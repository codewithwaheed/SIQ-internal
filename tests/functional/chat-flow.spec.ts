...
test.describe('Chat Flow', () => {
  ...
  test('should handle basic user question and AI response', async ({ page }) => {
  ...
  test('should handle escalation flow', async ({ page }) => {
    ...
    await page.fill('[data-testid="chat-input"]', 'I need help with incident response planning.');
    await page.click('[data-testid="send-button"]');
    ...
    await page.fill('[data-testid="escalation-reason"]', 'Need detailed incident response plan review');
    ...
    const confirmationText = await page.textContent('[data-testid="escalation-success"]');
  ...
  test('should save conversation history', async ({ page }) => {
    ...
    await page.fill('[data-testid="chat-input"]', 'First message about security frameworks');
    await page.click('[data-testid="send-button"]');
    await page.waitForSelector('[data-testid="ai-response"]');
    
    await page.fill('[data-testid="chat-input"]', 'Second message about compliance');
    await page.click('[data-testid="send-button"]');
    await page.waitForSelector('[data-testid="ai-response"]');
  ...
  test('should handle long messages properly', async ({ page }) => {
    ...
    const longMessage = 'This is a very long message that tests the chat interface handling of extended content. '.repeat(50);
    ...
    const displayedMessage = await page.textContent('[data-testid="user-message"]');
  ...
  test('should handle document upload in chat', async ({ page }) => {
    ...
    await page.fill('[data-testid="chat-input"]', 'Analyze the uploaded document');
    await page.click('[data-testid="send-button"]');
